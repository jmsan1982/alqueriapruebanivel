<?php
	class campus_muro {
		...
		public static function findDatosCampusMuroById($id) {
			
			return db_utf8::singleton()->query('SELECT * FROM registros_campus_murosuenyos WHERE id=' . $id)->fetch();
		}

		public static function damedatosCampusMuro($numero) {
			//error_log('SELECT * FROM registros_campus_murosuenyos WHERE numeropedido=' . '"'.$numero.'"');
			return db_utf8::singleton()->query('SELECT * FROM registros_campus_murosuenyos WHERE numeropedido=' . '"'.$numero.'"')->fetch();
		}


		//Para contar los registros de cada opcion (mañana, dia completo o dias sueltos) y mostrarlos en el back
		public static function findInscripcionesCampusMuroByOpcion($filtro)
        {
			//error_log(print_r('SELECT count(*) as total FROM registros_campus_murosuenyos WHERE opcion ='.'"'.$filtro.'"',1));
			return db_utf8::singleton()->query('SELECT count(*) as total FROM registros_campus_murosuenyos WHERE eliminado=0 and  fecharegistro>"2021-02-01" and opcion ='.'"'.$filtro.'"')->fetch();
		}



		//sacamos las inscripciones activas de la temporada correspondiente por fecha actual
		public static function findInscripcionesCampusMuroActivas()	{			
			$query = db_utf8::singleton()->query('SELECT * FROM registros_campus_murosuenyos WHERE eliminado=0 AND fecharegistro>"2021-02-01" ORDER BY numeropedido DESC');
		...
		public static function findAllInscripcionesExcelCampusMuro()	{			
			/*$query = db_utf8::singleton()->query('SELECT fecharegistro, numeropedido,  
				convert(cast(convert(opcion using latin1) AS binary) using utf8) AS opcion, 
				concat (convert(cast(convert(nombre using latin1) AS binary) using utf8), " ", convert(cast(convert(apellidos using latin1) AS binary) using utf8)) AS Inscrito, 
				dni, fechanacimiento, turno1, turno2, turno3, turno4, 
				convert(cast(convert(club using latin1) AS binary) using utf8) AS club, 
				convert(cast(convert(observaciones using latin1) AS binary) using utf8) AS observaciones, telefonotutor, 
				concat (convert(cast(convert(nombretutor using latin1) AS binary) using utf8) , " ", convert(cast(convert(apellidostutor using latin1) AS binary) using utf8) )AS Tutor, 
				emailtutor, dnitutor, direccion, numero, piso, puerta, cp, 
				convert(cast(convert(provincia using latin1) AS binary) using utf8) AS provincia, 
				convert(cast(convert(poblacion using latin1) AS binary) using utf8) AS poblacion
				autorizodatos, autorizoimagen, if(pagado=1, "Si", "No") AS pagado, tipo_pago, importe, sintomascovid, familiarcovid FROM registros_escuela_pascua WHERE eliminado = 0 AND fecharegistro > "2021-02-01" ORDER BY numeropedido DESC');*/

			$query = db_utf8::singleton()->query('SELECT fecharegistro as FechaRegistro, numeropedido as Pedido, opcion as Opcion,  genero as Genero, 
				concat (nombre, " ", apellidos) AS Inscrito, fechanacimiento as FechaNac, dni as Dni, club as Club,   observaciones AS ObserMedicas,   
				concat (nombretutor, " ", apellidostutor) AS Tutor, telefonotutor as Telefono, emailtutor as Email, dnitutor as DniTutor, direccion AS Direccion, numero, piso, puerta, cp, provincia as Provincia, poblacion AS Poblacion, autorizodatos, autorizoimagen, tipo_pago, if(pagado=1, "Si", "No") AS Pagado, importe, sintomascovid, familiarcovid FROM registros_campus_murosuenyos WHERE eliminado=0 and fecharegistro>"2021-02-01" ORDER BY numeropedido DESC');
		...
		// Para la consulta de exportar a excel
		public static function findAllInscripcionesExcelCampusMuroConCampos($where, $campoorder, $campos)	{

			$query = db_utf8::singleton()->query('SELECT fecharegistro, numeropedido as Pedido, opcion as Opcion, genero as Genero, concat (nombre, " ", apellidos) AS Inscrito, fechanacimiento,  '. $campos .'    concat (nombretutor, " ", apellidostutor) AS Tutor, telefonotutor as Telefono, emailtutor As Email,  if(pagado=1, "Si", "No") AS Pagado, importe as Importe, tipo_pago as TipoPago FROM registros_campus_murosuenyos WHERE eliminado=0 and fecharegistro>"2021-02-01"'. $where .'ORDER BY '. $campoorder .' DESC');
		...
		public static function actualizapagadoCampusMurotpv($codigo, $pagado) {
			$sql = "UPDATE registros_campus_murosuenyos SET pagado=:pag  WHERE numeropedido=:numero";
		...
		//correcto
		public static function ActualizaPagadoCampusMuro($id, $pagado) {
			$sql = "UPDATE  registros_campus_murosuenyos SET pagado=:pag WHERE id=:numero";
			...
			if (!$query->execute($datos)) {
				...
				echo "<script text='javascript'>
						...
						window.location.replace('?r=campusmuro/BackCampusMuro');
					</script>";
				die;
			}
		}
		...
		public static function actualizacodigoerror($pedido, $codigo_error) {
			$sql = "UPDATE registros_campus_murosuenyos SET errorcode=:error  WHERE numeropedido=:numero";
			$query = db::singleton()->prepare($sql);
		...
		public static function DardeBajaCampusMuro($pedido) {
			$sql = "UPDATE registros_campus_murosuenyos SET eliminado = 1 WHERE numeropedido = :numero";
			$query = db::singleton()->prepare($sql);
		...
		public static function comprobarRepetidos() {
			return db::singleton()->query('SELECT * FROM registros_campus_murosuenyos where eliminado=0 and fecharegistro>"2021-01-01"')->fetchAll();
		}


		public static function findLastNumeroPedidoCampusMuro() {
			return db::singleton()->query('SELECT MAX(id) FROM registros_campus_murosuenyos')->fetch();
		}


		public static function newFormCampusMuro($fecharegistro,$opcion,$nombre,$apellidos,$fechanacimiento,$dni,$club,$observaciones,$fotocopiasipsubida,$nombretutor,$apellidostutor,$dnitutor,$direccion,$numero,$piso,$puerta,$cp,$provincia,$poblacion,$telefonotutor,$emailtutor,$autorizodatos,$autorizoimagen,$numeropedido,$importe,$dia1,$dia2,$dia3,$dia4,$tipopago, $sintomasc, $familiarc, $genero, $pagado, $ficherosubido2) {

			$sql = "INSERT INTO registros_campus_murosuenyos(fecharegistro,opcion,nombre,apellidos,fechanacimiento,dni,club,observaciones,fotocopiasip,nombretutor,apellidostutor,dnitutor,direccion,numero,piso,puerta,cp,provincia,poblacion,telefonotutor,emailtutor,autorizodatos,autorizoimagen,pagado,numeropedido,importe,evento, turno1, turno2, turno3, turno4, tipo_pago, sintomascovid, familiarcovid, genero, resguardopago)
			VALUES (:fecharegistro, :opcion, :nombre, :apellidos,:fechanacimiento,:dni,:club,:observaciones,:fotocopiasipsubida, :nombretutor,:apellidostutor,:dnitutor,:direccion,:numero,:piso,:puerta,:cp,:provincia,:poblacion,:telefonotutor, :emailtutor, :autorizodatos, :autorizoimagen, :pagado, :numeropedido, :importe, :evento, :t1,:t2,:t3,:t4, :tipopag, :sint, :famili, :gen, :resg)";
		...
		public static function updatefichaCampusMuro($idinscripcion,
            $dnijugador,
            $nombre,    $apellidos, $date,  $club,  $direccion, 
            $numero, $piso,         $puerta,        $poblacion, $cp, $prov, 
            $nombretutor, $apeltutor, $dnitutor, $tlftutor, $email,  
                $observ,  
             $sip, $sintomascovid, $familiarcovid, $resguardo, $opcion, $importe, $turno1 , $turno2 , $turno3 , $turno4, $genero, $tipopago)
		{
			//error_log("id_inscrip en update escuela fallas= ".$idinscripcion);
            
			$sql = "UPDATE  registros_campus_murosuenyos 
                    SET     dni=:dnij, 
                            ...
                            nombretutor=:nomtut,    apellidostutor=:apeltut,    dnitutor=:dnitut,           telefonotutor=:telef, 					emailtutor=:email, 
                             ...
                             fotocopiasip=:sip ,  resguardopago=:resg,  opcion=:opc, turno1=:t1, turno2=:t2, turno3=:t3, turno4=:t4,  importe=:precio, genero=:gen, tipo_pago=:pago 
                    WHERE 
                            id=:idinscrip";
			$query = db_utf8::singleton()->prepare($sql);
			...
			if (!$query->execute($datos)) {
				...
				//error_log("id_inscrip ok en update campus muro= ".$idinscripcion);
				return true;
			}
		}
		...
		public static function mailssend($numeropedido, $contenido, $seccioninscripcion, $email) {
			...
			$mail->AddAddress($email, 'Inscripción en Campus Muro de los Sueños 2021');

			// Con copia oculta
			$mail->AddBCC('campus@valenciabasket.com', 'Inscripción en Campus Muro de los Sueños 2021');
			$mail->AddBCC('recepcion@alqueriadelbasket.com', 'Inscripción Campus Muro de los Sueños 2021');
