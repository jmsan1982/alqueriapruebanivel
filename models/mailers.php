<?php
	class mailers {
		...
		// Pablo: Creo este método a partir de updateForm porque aquel no actualiza el número de pedido
		...
		// Hace la inserción en la tabla "formulariosinscripciones_pagos". Así, cada inscripción tiene 1 fila que sirve para gestionar los pagos de la inscripción
		...
		// Envío de emails de confirmación Jornadas Detección
		public static function mailsSendJornadasDeteccion($emailtutor, $contenido, $opcion, $seleccionado_a, $hijo_a) {
          ...
          // $mail->SMTPSecure = "tls";   
			...
			// Indicamos la dirección del destinatario
			...
			// Ponemos aquí el asunto del email
			$mail->Subject = "Ficha inscripción Jornadas de Detección 2020";
			...
			$plantilla_inferior =			'<p style="margin-bottom:0;">
												En caso de cualquier duda o problema con la inscripción, remitan un correo a <u><a href="mailto:recepcion@alqueriadelbasket.com" style="color:#201f1e;">recepcion@alqueriadelbasket.com</a></u>
											</p>
										</td>
									</tr>
									<tr>
			...
			// Definimos el mensaje dinámico para los primeros 20 inscritos de cada selección
			$plantilla_inscrito = '
			<h3 style="color:#eb7c00;border-bottom:#eb7c00 2px solid">Ficha inscripción Jornadas de Detección 2020</h3>
			<p>Hola,</p>
			<p>Te confirmamos que has inscrito correctamente a tu '.$hijo_a.' a la <strong>'.$num_jornada.' Jornada de Detección</strong>, la cual se celebrará en esta temporada 2019/2020 en la Escuela del Valencia Basket Club el día <strong>'.$fecha_jornada.'</strong>.</p>
			<p>Agradecemos vuestro interés en realizar los entrenamientos previstos y te informamos que, tras la celebración de la última Jornada prevista para el 5 de julio de 2020, te comunicaremos en un plazo de 10 días si tu '.$hijo_a.' ha sido '.$seleccionado_a.' para formar parte de la Escuela del Valencia Basket Club para la temporada 2020/2021. En el caso de que no haya sido '.$seleccionado_a.' también recibirás un email agradeciéndole su participación en las mismas.</p>
			<p>Para que tu '.$hijo_a.' pueda participar solo tienes que rellenar, firmar y enviarnos a <u><a href="mailto:recepcion@alqueriadelbasket.com" style="color:#201f1e;">recepcion@alqueriadelbasket.com</a></u> esta <a href="https://servicios.alqueriadelbasket.com/pdf/autorizacion-jornada-deteccion-2019-2020.pdf" target="_blank" title="Descargar" style="font-weight:bold;font-style:italic;color:#ed8817;">autorización de asistencia</a> antes del día de la prueba.</p>
			<p>Tu '.$hijo_a.' deberá acudir con ropa deportiva para poder realizar una sesión de entrenamiento dirigida por nuestros entrenadores.</p>
			<p>En el caso de que por cualquier circunstancia no pudiera asistir al entrenamiento, te rogamos que lo avises mandando un em@il a la misma dirección, <u><a href="mailto:recepcion@alqueriadelbasket.com" style="color:#201f1e;">recepcion@alqueriadelbasket.com</a></u>, indicando vuestros datos. ¡Gracias!</p>
			<p>Estos son los datos que habéis introducido:</p>';

			// Definimos el mensaje para los que pasan a la lista de espera
			$lista_espera = '
			<h3 style="color:#eb7c00;border-bottom:#eb7c00 2px solid">Ficha inscripción Jornadas de Detección 2020</h3>
			<p>Hola,</p>
			<p>Ante todo, ¡gracias por contar con nosotros!<br>Te informamos que en este momento no quedan plazas disponibles para la generación de tu '.$hijo_a.' en la '.$num_jornada.' Jornada de Detección. No obstante, <u><strong>quedáis incluidos en una lista de espera</strong></u> para la Jornada indicada y en el caso de que hubiese una vacante por parte de alguno de los participantes, tanto por lesión como por cualquier otra circunstancia similar, os avisaríamos para que pudierais asistir.</p>
			<p>En el caso de que deseéis apuntaros a otra Jornada de Detección, deberéis rellenar de nuevo el <a href="https://servicios.alqueriadelbasket.com/?r=index/JornadasDeteccion" target="_blank" title="Descargar" style="font-weight:bold;font-style:italic;color:#ed8817;">formulario de inscripción</a>.</p>
			<p>Estos son los datos que habéis introducido:</p>';

			// Día a partir del cual se empiezan a contabilizar las inscripciones
			$apertura_inscripciones = "2020-01-01";

			// Contamos los registros totales para una generación de una Jornada concreta
			...
			// Muestra en el log cuantos inscritos hay en la opción seleccionada
			error_log(print_r($inscritos_jornada_seleccionada, 1));

			// Si, después de añadirlo a la BBDD, hay 20 o menos inscritos con la opción seleccionada, entonces se envía email de inscrito
			if ($inscritos_jornada_seleccionada[0] <= 20) {
				$body = $plantilla_superior.$plantilla_inscrito.$contenido.$plantilla_inferior;
			}
			// Si hay más de 20 inscritos con la opción seleccionada, entonces se envía email de lista de espera
			else {
		...
		// Envío de emails de confirmación Campus Navidad
		/*public static function mailsSendCampusNavidad($emailtutor, $contenido) {
			...
			// Indicamos la dirección y el nombre del destinatario
			$mail->AddAddress($emailtutor, 'Inscripción al Campus de Navidad');

			// Con copia oculta
			$mail->AddBCC('campus@valenciabasket.com', 'Inscripción al Campus de Navidad');
			$mail->AddBCC('recepcion@alqueriadelbasket.com', 'Inscripción al Campus de Navidad');

			// Ponemos aquí el asunto
			$mail->Subject = "Confirmación de inscripción al Campus de Navidad";
		...
		// Envío de correo de confirmación Adidas Next Generation
        ...
        // $mail->SMTPSecure = "tls";   
			...
			// Indicamos la dirección y el nombre del destinatario
			$mail->AddAddress($emailtutor, 'Solicitud de inscripción');

			// Con copia oculta
			$mail->AddBCC('imartinez@valenciabasket.com', 'Inscripción Adidas Next Generation');
			$mail->AddBCC('recepcion@alqueriadelbasket.com', 'Inscripción Adidas Next Generation');

			// Ponemos aquí el asunto
			$mail->Subject = "Confirmación de inscripción Adidas Next Generation";
		...
		// Envío de emails a Pepe para una Reserva de Pista desde menú back "Consulta Pistas"
		...
		// Envío de emails a Pepe para una ANULACION de Pista desde ?r=reservasusuarios/BackGestionUser
		public static function enviaCorreoAnulacionPistaAPepe($entrenador, $email, $contenido) {
           ...
           // $mail->SMTPSecure = __phpmailer_smtpsecure__;   
			...
			// Ponemos aquí el asunto
			$mail->Subject = 'Solicitud anulación de pista desde Servicios';
		...
		// Envío de emails a Pepe para un Cambio de Partido desde menú back "Consulta Pistas"
		...
		// Envío de emails a Pepe para una Reserva de Cámara / Máquina de tiro desde menú back "Consulta Pistas"
		public static function enviaCorreoCamaraMaquinaAPepe($nombre_solicitante, $email_solicitante, $contenido) {
           ...
           // $mail->SMTPSecure = __phpmailer_smtpsecure__;   
			...
			// Con copia oculta
			$mail->AddBCC('pcasares@alqueriadelbasket.com', 'Pepe Casares');
			$mail->AddBCC('recepcion@alqueriadelbasket.com', 'Recepción de L´Alqueria');
			//$mail->AddBCC('amomplet@tessq.com', 'Alex Momplet');

			// Ponemos aquí el asunto
			$mail->Subject = 'Solicitud de Cámara / Máquina de tiro desde Servicios';
		...
		// Envío de emails a Pepe para una Reserva de Sala desde menú back "Consulta Salas"
		...
		// Envío de emails a solicitantes de Federación por falta de datos
		public static function enviaCorreoAInscripciones($email, $mensaje) {
          ...
          // $mail->SMTPSecure = "tls";   
			...
			// Con copia
			//$mail->AddCC('imartinez@valenciabaset.com', 'Valencia Basket Inscripción Adidas');
			//$mail->AddBCC('recepcion@alqueriadelbasket.com', 'Inscripción Adidas Next Generation');
		...
		// Envío de emails a Recepción desde Solicitudes de Licencia
		public static function enviaCorreoARecepcion($mensaje){
           ...
           // $mail->SMTPSecure = "tls";   
			...
			<div style="width: 80%;padding: 2em;">
				<h2 style="text-align: center;color:#eb7c00;border-bottom: #eb7c00 2px solid;">Nueva solicitud de licencia de federación: </h2>
