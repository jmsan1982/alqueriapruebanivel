<?php
	class formulariosController	{
		...
		// Recibe el formulario de inscripción de las Jornadas de Detección
		public function actionJornadasDeteccionForm() {
			echo "<script text='javascript' charset='utf-8'>
					alert('El formulario de inscripción a las Jornadas de detección está bloqueado.');
			...
			if (isset($_POST['nombre'])) {
				...
				// Sacamos el año de nacimiento
				$anyonacimiento = date("Y", strtotime($_POST['fechanacimiento']));

				// Comprobamos si la generación que se ha seleccionado, en una jornada en concreto, concuerda con el año de nacimiento
				...
				if ($correcto == 0) {
					echo "<script text='javascript' charset='utf-8'>
							alert('Lo sentimos pero la fecha de nacimiento no corresponde con la generación seleccionada. ¡Gracias por contar con nosotros!');
				...
				// Creamos variables para personalizar el género en el email
				if ($genero == "Masculino") {
				...
				$contenido = "<p><strong>Fecha de registro:</strong> ".date("d/m/Y H:i", strtotime($fecharegistro))."h
					<br><strong>Opción:</strong> ".$opcion."
					...
					<br><strong>Teléfono tutor:</strong> ".$telefonotutor."
				...
				if ($nuevoformulario) {
					...
					//error_log(__FILE__.__FUNCTION__.__LINE__);
					echo "<script text='javascript' charset='utf-8'>
							alert('Parece que hubo algún error. Por favor, inténtalo de nuevo más tarde.');
							window.location.replace('?r=index/Principal');
						</script>";
					die;
				}
				...
				if ($nuevocorreo) {
					//error_log(__FILE__.__FUNCTION__.__LINE__);
					echo "<script text='javascript' charset='utf-8'>
							alert('¡Gracias, la solicitud de inscripción se ha realizado correctamente! En breve recibirás un correo con los datos introducidos.');
							window.location.replace('?r=index/Principal');
						</script>";
					die;
				}
		...
		// Recibe el formulario de inscripción del Skills Camp 2019
		public function actionSkillsCampForm() {
			...
			if (isset($_POST['nombre'])) {
				...
				// Sacamos el año de nacimiento (PARA NIN@S ENTRE 13 Y 19 AÑOS)
				$anyonacimiento = Date( "Y" , strtotime( $_POST['fechanacimiento'] ) );
				...
				if ($correcto == 0) {
					echo json_encode(array(
                        "response"      =>  "error",
                        "message"       =>  "La edad para poder registrarse es entre 13 y 19 años."));
                    die;
				}
				...
				switch ( $_POST['opcion'] ) {
				    ...
				    case "pack_dos_opcion_a":
				        $importe=920;
				   		$evento="Skills Camp y Tecnificación Femenina 2020";
				    break;
				}
		...
		*********************************/
		...
		// Recibe el formulario de inscripción del Campus Improve Basketball Talent 2019
			...
			if (isset($_POST['nombre'])) {
				...
				if ($edad >= 16 && $edad <= 20) {
					...
					if (isset($_POST['semana1'])) {
						// Inicializamos a 0 para ir sumándole el precio de las opciones elegidas de cada semana
						$importe = 0;
				...
				// Si ha nacido fuera del rango de años permitido, mostramos mensaje de prohibición
					...
					echo "<script text='javascript' charset='utf-8'>
							alert('Lo sentimos pero el jugador/a debe tener entre 16 y 20 años para participar en el campus. ¡Gracias por contar con nosotros!');
							window.location.replace('?r=index/ImproveTalent');
						</script>";
					die;
				}
		...
		********************************************************/
		...
		// Recibe el formulario de inscripción de la Liga de L´Alqueria del Basket https://servicios.alqueriadelbasket.com/?r=index/ligaalqueria
		public function actionLigaAlqueriaForm() {
			...
			/************************************************************
			*   1. Procesamos la información del equipo + responsable
			***********************************************************/
			...
			if (!empty($equipoExistente)) {
				...
				echo json_encode(array(
					"response"  =>  "error",
					"message"   =>  "Ya existe una inscripción con este DNI del responsable. Espere 5 minutos."));
				die;
			}
			...
			/*************************************************************************************
			*   2. Ahora generamos las 4 filas de los 4 pagos que deben hacer según el dossier 
			*************************************************************************************/

			$numero_pedido="";
			$numero_pedido="LIGA".$nuevoEquipo['ID'];
			
			formularios_liga_alqueria_pagos::insert($nuevoEquipo['ID'], "Inscripción y fianza", $numero_pedido, 200,    $metodo_pago,               null, null, 1);
			...
			****************************************************/
			...
			if (isset($_FILES["justificante_pago"]["tmp_name"])) {
				...
				if (move_uploaded_file($_FILES["justificante_pago"]["tmp_name"],$url_subida)) {
					// Además de mover el fichero, ahora comprobamos si hay que renombrarlo.
					...
					echo json_encode(array(
						"response" => "error",
						"message"  => "Parece que hubo algún problema con el archivo"));
				}
			}
									
			/*************************************************
			* Ahora está todo insertado. Vamos al TPV 
			*************************************************/
			...
			if ($metodo_pago=="TPV") {
				...
				echo json_encode(array(
						...
						"message"           =>  "La inscripción en la Liga Alqueria se ha recibido correctamente"));
			}
			else if ($metodo_pago=="OFICINAS") {
				//  Aquí enviamos el email de confirmación OFICINAS
				$contenido_email= "<p>Hemos recibido la inscripción a la liga de L´Alqueria del Basket. Estos son los datos recibidos:</p>
					<p>
						<strong>Fecha de alta:</strong> ".$FechaAlta."<br>
						<strong>Nombre del responsable:</strong> ".$ResponsableNombre."<br>
						<strong>Teléfono del responsable:</strong> " . $ResponsableTelefono."<br>
						...
						<strong>Equipación principal:</strong> ".$ColorEquipacionPrincipal."<br>
						<strong>Equipación secundaria:</strong> ".$ColorEquipacionSecundaria."<br>
						<strong>Día del partido:</strong> ".$DiaJuego."<br>
				...
				//  Devolvemos OK. La redirección a la página de confirmación se hace en el JavaScript
				echo json_encode(array(
					"response"          => "success",
		...
		*************************************************************/
		...
		// Recibe el formulario de inscripción al campus de navidad L´Alqueria del Basket https://servicios.alqueriadelbasket.com/?r=index/campusnavidad
		public function actionCampusNavidad2019Form() {
			...
			/************************************************************
			*   1. Procesamos la información del jugador y tutor
			***********************************************************/
			$FechaAlta                  =   date("Y-m-d H:i:m");
			...
			***********************************************************/
			...
			/**************************************************************
			 * 6. Redirijimos al TPV o mostramos mensaje de confirmación 
			 **************************************************************/
			if($tipopago=="TPV")
			{
				echo json_encode(array(
					...
					"message"           =>  'La inscripción en el campus de navidad se ha recibido.'));
			}
			else if($tipopago=="OFICINAS")
			{
				//  Aquí enviamos el email de confirmación OFICINAS. Hay que editar el contenido. 
				/*  $contenido_email= "<p>Hemos recibido la inscripción a la liga de L'Alqueria del Basket. Estos son los datos recibidos:</p>
					<p>
						<strong>Fecha de alta:</strong> ".$FechaAlta."<br>
						<strong>Nombre del responsable:</strong> ".$ResponsableNombre."<br>
		...
		public function actionOk() {
			...
			if (isset($_SESSION['actualizar_entidad_pagos'])) {
				...
				$enviodecorreo = skillcamp::mailssend($contenidocorreo['numero_pedido'], $contenido, "Ficha inscripción Campus Worcester", $contenidocorreo['email']);
				$enviodecorreo2 = skillcamp::mailssend($contenidocorreo['numero_pedido'], $contenido, "Ficha inscripción Campus Worcester", "campusworcester@valenciabasket.com");
		...
		/*  sanitize_nombre_para_columna_myslq()    indica si el string enviado por parámetro cumple la regex para atributos mysql:
		*  Acepta: 
		*      a-z
		*      A-Z
		*      0-9
		*      _
		*  Más información en https://regexr.com/   */
		...
		/************************************************************************
		I jornada de formación de la Cátedra de baloncesto L’Alqueria del Basket
		************************************************************************/
		...
		********************************************************/
		...
		public function actionIbiza() {
			...
			// Si ha nacido fuera del rango de años permitido, mostramos mensaje de prohibición
				...
				echo "<script text='javascript' charset='utf-8'>
						alert('Lo sentimos pero el niño/a debe tener entre 5 y 17 años para participar en la escuela. ¡Gracias por contar con nosotros!');
		...
		public function actionOkIbiza() {
            ...
            if ($contenidocorreo['semana1'] == "sem1_comp") {
                $semanas .= "-Semana del 27 de Julio al 1 de Agosto día completo";
            }
            else if ($contenidocorreo['semana1'] == "sem1_manyana") {
                $semanas .= "-Semana del 27 de Julio al 1 de Agosto solo mañanas";
            }
            else if ($contenidocorreo['semana1'] == "sem1_pension") {
                $semanas .= "-Semana del 27 de Julio al 1 de Agosto pensión completa";
            }
            elseif ($contenidocorreo['semana1'] == "sem1_comp/matinal") {
                $semanas .= "-Semana del 27 de Julio al 1 de Agosto día completo y escuela matinal";
            }
            else if ($contenidocorreo['semana1'] == "sem1_manyana/matinal") {
                $semanas .= "-Semana del 27 de Julio al 1 de Agosto solo mañanas y escuela matinal";
            }
            else if ($contenidocorreo['semana1'] == "sem1_pension/matinal") {
                $semanas .= "-Semana del 27 de Julio al 1 de Agosto pensión completa y escuela matinal";
            } 


            if ($contenidocorreo['semana2'] == "sem2_comp") {
                $semanas .= "-Semana del 3 al 8 de Agosto día completo";
            }
            elseif ($contenidocorreo['semana2'] == "sem2_manyana") {
                $semanas .= "-Semana del 3 al 8 de Agosto solo mañanas";
            }
            else if ($contenidocorreo['semana2'] == "sem2_pension") {
                $semanas .= "-Semana del 3 al 8 de Agosto pensión completa";
            }
            else if ($contenidocorreo['semana2'] == "sem2_comp/matinal") {
                $semanas .= "-Semana del 3 al 8 de Agosto día completo y escuela matinal";
            }
            elseif ($contenidocorreo['semana2'] == "sem2_manyana/matinal") {
                $semanas .= "-Semana del 3 al 8 de Agosto solo mañanas y escuela matinal";
            }
            else if ($contenidocorreo['semana2'] == "sem2_pension/matinal") {
                $semanas .= "-Semana del 3 al 8 de Agosto pensión completa y escuela matinal";
            }


            if ($contenidocorreo['semana3'] == "sem3_comp") {
                $semanas .= $semanas." -Semana del 10 al 15 de Agosto día completo";
            }
            elseif ($contenidocorreo['semana3'] == "sem3_manyana") {
                $semanas .= $semanas." -Semana del 10 al 15 de Agosto solo mañanas";
            }
            else if ($contenidocorreo['semana3'] == "sem3_pension") {
                $semanas .= "-Semana del 10 al 15 de Agosto pensión completa";
            }
            else if ($contenidocorreo['semana3'] == "sem3_comp/matinal") {
                $semanas .= $semanas." -Semana del 10 al 15 de Agosto día completo y escuela matinal";
            }
            elseif ($contenidocorreo['semana3'] == "sem3_manyana/matinal") {
                $semanas .= $semanas." -Semana del 10 al 15 de Agosto solo mañanas y escuela matinal";
            }
            else if ($contenidocorreo['semana3'] == "sem3_pension/matinal") {
                $semanas .= "-Semana del 10 al 15 de Agosto pensión completa y escuela matinal";
            }
            ...
            $enviodecorreo = ibiza::mailssend($contenidocorreo['numeropedido'], $contenido, "Ficha inscripción pago online Campus Ibiza", $contenidocorreo['emailtutor']);
		...
		public function actionokpagooficinaibiza() {
            ...
            $enviodecorreo = ibiza::mailssend($contenidocorreo['numeropedido'], $contenido, "Ficha inscripción pago oficina Campus Ibiza", $contenidocorreo['emailtutor']);
