<?php
	class escuelanavidadController {
		...
		public function actionOk() {
			...
			$actualizaestado = escuela_navidad::actualizapagadoescuelavnavidad($codigo, $pagado);

			$contenidocorreo = escuela_navidad::damedatosescuelanavidad($codigo);
			...
			$enviodecorreo = escuela_navidad::mailsSendEscuelaNavidad($contenidocorreo['numeropedido'], $contenido, "Ficha inscripción pago online Escuela Navidad de L´Alqueria", $contenidocorreo['emailtutor']);
		...
		public function actionOkpagoOficina() {
			...
			$enviodecorreo = escuela_navidad::mailsSendEscuelaNavidad($contenidocorreo['numeropedido'], $contenido, "Ficha inscripción pago oficina Escuela Navidad de L´Alqueria", $contenidocorreo['emailtutor']);
		...
		public function actionKo() {
			...
			$actualizaestado = escuela_navidad::actualizaerrortpvescuelavnavidad($pedido, $error);
		...
		public function actionImprimirPdf(){
			...
			$contenido = "<b>Estos son los datos recibidos relacionados con su inscripción. </b>".
                "<br><br><b>-Inscripcion en Escuela de Navidad 2020 nº: </b>".$contenidocorreo['numeropedido'].
			...
			"<br><b>-Opción de pago: </b>". $contenidocorreo['tipopago'];
		...
		public function actionEscuelaNavidadForm() {
			if (isset($_POST['nombre'])) {
            	...
            	$fechalimite = DateTime::createFromFormat('Y-m-d', "2020-12-23");
            	...
            	if ($edad >= 5 && $edad <= 11) {
                	...
                	$todoslosregistros = escuela_navidad::comprobarRepetidos();
					...
					$dir_subida = 'img/inscripcionesescuelanavidad/';
					...
					if ($_FILES['resguardoingreso']['error'] == 0) {
						...
						$fichero2 		= $dnitutor . "-imagen-2.jpg";
                	...
                	$numeropedido = $numeropedido . "enav20";
	                ...
	                if ($enviar=="tarjeta") {
	                    ...
	                    if ($nuevoformulario) {
	                        ...
	                        //header('Location: http://localhost/serviciosalqueria/tpv_escuelanavidad/tpv.php?pedido=' . $numeropedido . '&titular=' . $nombreapellidos . '&importe=' . $importe);
	                    }
	                    else {
	                    ...
	                    if($nuevoformulario){
	                      // header('Location: http://localhost/serviciosalqueria3/?r=escuelanavidad/okpagooficina&pedido=' . $numeropedido);
            ...
            // Si ha nacido fuera del rango de años permitido, mostramos mensaje de prohibición
                ...
                echo "<script text='javascript' charset='utf-8'>
                        alert('Lo sentimos pero el niño/a debe tener entre 5 y 11 años para participar en el campus. ¡Gracias por contar con nosotros!');
                        window.location.replace('?r=index/Principal'); </script>";
                die;
            	}
		...
		public function actionDardeBajaEscuelaNavidad()
    	{
	        ...
	        if (isset($_POST['id'])) {
	            ...
	            echo "<div style='width:100%;height:100%;background-color: rgba(0,0,0,.8);padding-top:10%;'>
							<div class='contact-form-wrapper' style='width:50%;margin-left:25%;background-color:white;border:1px solid #000000;border-radius: 10px 10px 10px 10px;padding:25px;'>
								<form action='?r=escuelanavidad/DardeBajaEscuelaNavidad' method='post'>
	        ...
	        if (isset($_POST['confirm'])) {
	            ...
	            if ($_POST['confirm'] == "SI") {
	                ...
	                $baja_reg = escuela_navidad::DardeBajaEscuelaNavidadAlq($pedido);
	            }
	            if ($_POST['confirm'] == "NO") {
	                echo "<script text='javascript'> window.location.replace('?r=campus/BackEscuelaNavidadAlq'); </script>";
	            }
	        } else {
	            require "error.php";
	        }
		...
		public function actionImprimirFichaEscuelaNavidadAlq() {
			...
			if (isset($_SESSION['usuario']) && ($_SESSION['rolusuario'] == "admin" || $_SESSION['rolusuario'] == "superadmin")) {
				...
				if ($tipopago == "OFICINA") {
					$seccioninscripcion = "Ficha inscripción pago oficina Escuela de Navidad de L´Alqueria";
				}
				elseif ($tipopago == "ONLINE") {
					$seccioninscripcion = "Ficha inscripción pago online Escuela de Navidad de L´Alqueria";
				}
				...
				$contenidocorreo = escuela_navidad::damedatosescuelanavidad($numero);
		...
		public function actionMostrarModalEscuelaNavidad()
	    {
	        ...
	        if (!empty($idinscripcion)) {
	            ...
	            $datos = escuela_navidad::findDatosEscuelaNavidadById($idinscripcion);
	            $alerta_cuenta = "";

	            echo json_encode(array(
	                "response" => "success",
	                "instancia" => $datos,
	                "modal_title" => "Formulario de inscripción en Escuela Navidad VB",
	     ...
	     public function actionUpdateInscripcionModalEscuelaNavidad()
    {
//            error_log(__FILE__.__FUNCTION__.__LINE__);
//            error_log(print_r($_POST,1));
      ...
      //  $alergias = filter_input(INPUT_POST, 'alergias', FILTER_SANITIZE_STRING);
       ...
       // $transporte = filter_input(INPUT_POST, 'transporte', FILTER_SANITIZE_STRING);
        ...
        if (!empty($_FILES['sipnuevo']['tmp_name'])) {
            ...
            $sip = 'img/inscripcionesescuelanavidad/' . strtolower(self::sanitize_nombre_para_columna_myslq($_POST['nombre'])) . "-" . strtolower(self::sanitize_nombre_para_columna_myslq($_POST['apellidos'])) . "-" . date("d-m-Y-H-i-s") . "." . $extension;
        ...
        if (isset($idinscripcion) && !empty($idinscripcion)) {
         ...
         //  error_log("inscripcion: " . $idinscripcion);
            $instancia = escuela_navidad::updatefichaescuelanavidad(
