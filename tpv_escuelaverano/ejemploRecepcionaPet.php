...
if (!empty( $_POST ) ) {//URL DE RESP. ONLINE
					...
					if ($firma === $signatureRecibida){
						...
						header('Location: https://servicios.alqueriadelbasket.com/?r=escuelaverano/ok&codigo='.$codigo_venta_recibido);
					} else {
						//echo "FIRMA KO. Error, firma incorrecta";
						
						header('Location: https://servicios.alqueriadelbasket.com/?r=escuelaverano/ko&codigoerror='.$codigoRespuesta.'&pedido='.$codigo_venta_recibido);
					}
	}
	else{
		if (!empty( $_GET ) ) {//URL DE RESP. ONLINE
			...
			if ($firma === $signatureRecibida){
				...
				if ($dos_char == "00") {
					...
					header('Location: https://servicios.alqueriadelbasket.com/?r=escuelaverano/ok&codigo='.$codigo_venta_recibido);
					//header('Location: http://localhost/alqueriaforms/?r=escuelaverano/Ok&codigo='.$codigo_venta_recibido);
				} else{
					
					header('Location: https://servicios.alqueriadelbasket.com/?r=escuelaverano/ko&codigoerror='.$codigoRespuesta.'&pedido='.$codigo_venta_recibido);
					//header('Location: http://localhost/alqueriaforms/?r=escuelaverano/ko');
				}
			} else {
				//echo "FIRMA KO";
