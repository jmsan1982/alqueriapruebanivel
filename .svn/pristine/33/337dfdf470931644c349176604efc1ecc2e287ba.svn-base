<?php
/******************************************************************
 * La carpeta /tpv_renov_inscripciones/ gestiona los pagos de:
 * INSCRIPCIONES a ESCUELA - cantera y patronato 2020
 *****************************************************************/
session_start();
...
$default_lang="cas";
if(!empty($_GET["lang"]) && ($_GET["lang"]==="cas" || $_GET["lang"]==="val" || $_GET["lang"]==="eng" )){  $default_lang=$_GET['lang'];}
...
//$url_base="https://servicios.alqueriadelbasket.com/"; 
$url_base="http://localhost/serviciosalqueria3/"; 

$url    = $url_base."tpv_renov_inscripciones/ejemploRecepcionaPet.php";     //  HILO 1: URL ejecutada por BOT del banco
$urlOK  = $url_base."index.php?r=inscripciones/InscripcionEscuela2020PagoTPVOK";    //  HILO 2: URL a la que se envia el usuario
$urlKO  = $url_base."index.php?r=inscripciones/InscripcionEscuela2020PagoTPVKO";    //  HILO 2: URL a la que se envia el usuario
...
//$_POST['concepto'];  //maximo 60 caracteres
$id         =   $_GET['pedido'];
$titular    =   $_GET['titular'];


//  Valores fijos
//$concepto   =   $lang["ins_escuela_tpv_concepto"];   //maximo 125 caracteres
//$amount     =   5000;   //importe a pagar 50€. Las dos ultimas posiciones se consideran decimales


/* Si viene de Patronato el importe se le pasa por GET */
if (isset($_GET['importe'])){
    $amount    =  $_GET['importe'];
    $amount    =  $amount."00";   //Las dos ultimas posiciones se consideran decimales

    $concepto  ="Reserva Inscripción Patronato L'Alqueria del Basket 20/21";

} else { //esto es para escuela y cantera
    $amount     =   5000;   //importe a pagar 50€  en escuela y cantera. Las dos ultimas posiciones se consideran decimales
    $concepto   =   $lang["ins_escuela_tpv_concepto"];   //maximo 125 caracteres
}
                    ...
                    <div class="b-wrap-club-gallery">
                        <div class="container">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="text-wrap">
                                        <h2 class="section-title"><span class="orange-text"><?php echo $lang["ins_escuela_tpv_1"];?></span></h2> 
                                    </div>
                                </div>

                                <div class="col-md-12 alqueria-bg-box" style="min-height:auto;background-color:#fafafa;">
                                    <h4 class="t-center"><?php echo $lang["ins_escuela_tpv_2"];?></h4>
                                    <h4 class="t-center"><?php echo $lang["ins_escuela_tpv_3"];?></h4>
                                    ...
                                    <form name="frm" action="<?php echo $url_formulario;?>" method="POST" target="_blank">
                                        ...
                                        <button class="btn btn-link-o" type="submit" id="submit">
                                            <span class="mt-0"><?php echo $lang["ins_escuela_tpv_4"];?></span>
                                        </button>
                                    </form>
                                </div>
                            </div>
