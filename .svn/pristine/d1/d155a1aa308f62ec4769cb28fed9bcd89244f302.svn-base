...
$.fn.calendar = function(options) {
	...
	var defaults = { // Opciones por defecto --> selectorPlaceCalendar: 'input[type="date"], input.date-picker'
		selectorPlaceCalendar: 'input.date-picker'
		...
		while (countCalendarDay<totalCalendarDays) {
			...
			if (countPreviousMonthDay>0 || countMonthDay>=numberOfDays) { // Previous and next months' days
				...
				if(fechacalendar>=fechahoyceros){
					tagLink.attr({'href':'?r=pistas/consultaSalas&fecha='+fechacalendar+'','data-date':''+year+'-'+textMonth+'-'+textDay+''});		
				}
