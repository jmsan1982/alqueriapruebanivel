$(document).ready(function() {
	...
	$.fn.calendar({
		textNavigationMonths: ['<img src="img/CAL_IMG_navigation_previous.png" alt="'+calendarLocale[languageSetting]['navigationMonths'][0]+': " style="height: 2.0em;width: 2.0em;margin-top: 20px;" />','<img src="img/CAL_IMG_navigation_next.png" alt="'+calendarLocale[languageSetting]['navigationMonths'][1]+': " style="height: 2.0em;width: 2.0em;margin-top: 20px;" />']
		...
		while (countCalendarDay<totalCalendarDays) {
			...
			if (countPreviousMonthDay>0 || countMonthDay>=numberOfDays) { // Previous and next months' days
				...
				if(fechacalendar>=fechahoyceros){
					tagLink.attr({'href':'?r=departamentotecnico/Aulavirtual&fecha='+fechacalendar+'','data-date':''+year+'-'+textMonth+'-'+textDay+''});		
				}
